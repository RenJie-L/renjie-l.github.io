...
import { M_PolarLight, texture_Star } from './Materials';
...
export class PolarLight extends Component {
  ...
  onLoad(): void {
    ...
    for (const i of meshList) {
      const t = {
        ...
        rotation: new Quaternion().setFromEuler(
          new Euler(i.Rotation[0], -i.Rotation[1], i.Rotation[2]),
        ),
        scale: new Vector3(0.1, 0.1, 0.1),
      };
      this.PolarLightTransforms.push(t);
    }
    const customSort = function (a: TransformInfo, b: TransformInfo) {
      return a.position.z - b.position.z;
    };
    this.PolarLightTransforms.sort(customSort);

    const v = this.viewer.user.resources.SM_Light as Object3D;
    ...
    v.traverse((node: Object3D) => {
      ...
      if (mesh instanceof Mesh) {
        const instancedMesh = new InstancedMesh(
          mesh.geometry,
          M_PolarLight,
          this.PolarLightTransforms.length,
        );
    ...
    this.viewer.addPlugin(starParticle, {
      PolarLightTransforms: this.PolarLightTransforms,
    });
  }
  update(dt: number): void {
    ...
    if (this.lightInst) {
      if (
        this.PolarLightTransforms[this.PolarLightTransforms.length - 1].position
          .z > cameraCenter.z
      ) {
  ...
  private updateInstance() {
    for (let i = 0; i < this.PolarLightTransforms.length; i++) {
      ...
      matrix.compose(
        this.PolarLightTransforms[i].position,
...
class starParticle extends Plugin {
  ...
  onLoad({
    PolarLightTransforms,
  }: {
    PolarLightTransforms: TransformInfo[];
  }): void {
  ...
  update(dt: number): void {
    ...
    const vpos =
      this.LightTransforms[this.LightTransforms.length - 1].position.clone();
