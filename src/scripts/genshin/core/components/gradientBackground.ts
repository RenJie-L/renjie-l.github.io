...
function inv_rrt_odt_fit(v: number): number {
  const a = -(
    Math.sqrt(10) *
      Math.sqrt(-187248350 * Math.pow(v, 2) + 232585567 * v + 241290) +
    21650 * v -
    1230
  );
...
function ACES_Inv(color: Vector3): Vector3 {
  const ACES_INPUT_MAT = new Matrix3();
  ACES_INPUT_MAT.set(
    1.76474,
    -0.67577,
    -0.08896,
  ...
  const ACES_OUTPUT_MAT = new Matrix3();
  ACES_OUTPUT_MAT.set(
    0.64304,
    0.31119,
    0.04578,
  ...
  onLoad(): void {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const l = 1 - y / (height - 1); // 计算当前位置在渐变色数组中的插值位置
        const t = 1 - this.smoothstep(0, tb_l, l);
        const tb =
          this.smoothstep(0, tb_l, l) * (1 - this.smoothstep(tb_l, b_l, l));
        const b = Math.pow(this.smoothstep(tb_l, b_l, l), 1);
